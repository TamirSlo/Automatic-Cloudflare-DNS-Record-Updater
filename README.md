...
## Dependencies
The commands `ping` and `ip` are used in the script, if they are not installed on your distribution by default, please install them using `apt install -y iputils-ping iproute2`
...
3. Create the `ip.txt` file in DNSUpdater with your current IPv4 Address.
4. Write down your Cloudflare EMail and API Key inside cloudflare.sh
5. Add a cron job:
